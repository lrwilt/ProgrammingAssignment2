...
## in variable x and then creates three closure functions that can house and
## return the inverse matrix created by cachSolve. makeCacheMatrix returns these
## three closure functions as a list that can be accessed by cacheSolve.
...
makeCacheMatrix <- function(x = matrix()) {
    ...
    ## used by cacheSolve to set m for this environment as the inverse of the matrix stored in x
    ...
    ) ##stores and returns the functions as a list, which can be accessed by cacheSolve
}
